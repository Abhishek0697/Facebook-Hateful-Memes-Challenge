...
class HatefulMemesFeaturesDataset(MMFDataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'text_senti' in sample_info:
            current_sample.text_senti = torch.tensor([sample_info['text_senti']], dtype=torch.float)
            current_sample.img_senti = torch.tensor(sample_info['img_senti'], dtype=torch.float)
        if 'caption' in sample_info:
            processed_text = self.text_processor({"text": sample_info["caption"]})
            current_sample.caption = processed_text["input_ids"]
